...
def CBOW(text, words_index, wcount, twcount, context=3, epoch = 10, d=50, neg=5):
    ...
    for ep in range(epoch):
        for line in text.split('\n'):
            ...
            while inc < llen-context:
                ...
                w = [w0] + [random.randrange(wcount) for i in range(neg)]
    ...
    pickle.dump([wweight, words_index], open('wd.pkl','wb'))
    return
...
if __name__ == '__main__':
    ...
    wcount += 1
    print
    CBOW(text, words_index=words_index, wcount=wcount, twcount=twcount)
